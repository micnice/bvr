...
if ((strcmp($checkbvr, 'BVR') == 0 && strlen($nationalid) == 10)
  ...
  $strSQL
      = "select coalesce(max(trim(substr(nationalid,4))::int),0) from beneficiarymaster where nationalid like 'BVR%' and nationalid not like '%NZ%' and length(trim(substr(nationalid,4)))<8";
