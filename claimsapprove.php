...
if ($vserial == 1) {
  $strSQL
      = "SELECT idvoucherclaims,claimdate,voucherserial,patientid,serviceoffered,amount,facilityname,shortname"
  ...
  for ($i = 0; $i < $numrows; $i++) {
    ...
    ?>
      <tr>
          ...
          <td>
              <a href="main.php?page=claimsapprove_process&dest=approve&approve=yes&id=<?php echo $row['idvoucherclaims'] ?>&distributor=<?php echo $distributor ?>&period=<?php echo $period ?>">Approve</a>
              |
              <a href="main.php?page=claimsapprove_process&dest=approve&approve=no&id=<?php echo $row['idvoucherclaims'] ?>&distributor=<?php echo $distributor ?>&period=<?php echo $period ?>">Reject</a>
          </td>
      </tr>
    <?php
  }
