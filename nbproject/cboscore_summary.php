   ...
   for($i = 0; $i < $numrows; $i++) {
    ...
    ?>
        <tr>
           ...
           <td><?php echo number_format(($row["score"]/($row["qnumber"]*45))*100,2,'.',''); ?> % </td>
           <td>
               <form action="viewcboquestionnaires.php" method="POST">
                <div align="center">
                    <input type="hidden" name="distributor" value="<?php echo $distributor; ?>">
                    <input type="hidden" name="period" value="<?php echo $row["periodid"]; ?>">
...
<hr />
<?php
    if(strcmp(trim($_SESSION['userrole']),'clinic')==0){
