...
<td><font size="4">Rejection Reasons:</font></td>
            <td>
                ...
                <?php
                $sql = pg_query($conn,"SELECT idreason,rejectshortname FROM rejectreasons order by rejectshortname");
                while ($row = pg_fetch_assoc($sql)) {
                echo '<option value="'.htmlspecialchars($row['idreason']).'">'.htmlspecialchars($row['rejectshortname']).'</option>';}
                ?>
                </select> 
            </td>
    </tr>
    <tr><td colspan="2" style="text-align: center;"><input type="submit" value="Reject Claim" /></td></tr>
</table>  
...
if(strcasecmp($approve, "reapp")==0){
    $query = "update voucherclaims set approval='yes',periodid=$period where idvoucherclaims=$id";
}elseif (strcasecmp($approve, "yes")==0) {
    $query = "update voucherclaims set approval='$approve' where idvoucherclaims=$id";
