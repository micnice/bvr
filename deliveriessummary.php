        ...
        <table>
            ...
            while ($facilityrow = pg_fetch_assoc($resultFacility)) {
                ...
                for ($x = $period; $x <= $period2; $x++) {
                    $sql = pg_query($conn, "SELECT monthcode,monthname,yearname FROM period where monthcode=$x");
                    $periodrow = pg_fetch_assoc($sql);
                    $periodname = trim($periodrow['monthname']) . " " . trim($periodrow['yearname']);
                    //echo $periodname.' , ';
                    ...
                    for ($i = 0; $i < $numrows; $i++) {
                        ...
                        ?>
                        <tr>
                            <td><a href="voucherredeemed.php?distributor=<?php echo $row[0]; ?>&facilityname=<?php echo urlencode(trim($row[1])); ?>"><?php echo $row[1]; ?> </a></td>
