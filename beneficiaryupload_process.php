...
//Upload File
	...
	if (is_uploaded_file($_FILES['beneficiary']['tmp_name'])) {
		echo "<h1>" . "File ". $_FILES['beneficiary']['name'] ." uploaded successfully." . "</h1>";
	}
...
//File Import format: Reg Date, Certificate No.,Surname ,Other names,National ID,DOB,Household Head ID Number,HH Firstname, HH Surname, Address,Suburb,Area,Phone Number,City
	...
	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {	
                ...
                if($idhousehold==''){$idhousehold='null';}
                
                $hhfirstname=$data[7];
                if($hhfirstname==''){$hhfirstname='null';}
                
                $hhsurname=$data[8];
                if($hhsurname==''){$hhsurname='null';}
		...
		$import="INSERT into beneficiarymaster (serialno,surname,firstname,dob,nationalid,idhousehold,location,village,area,city,phone,bvrnumber,reg_date,uploadtime,uploadfilename,addedby)" 
