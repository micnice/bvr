...
$strSQL
    = "SELECT * from redeemedvouchers where redeemserial='$voucherserial' and vouchertype=$vouchertype";
//echo $strSQL.'<br />';
...
if ($numrows == 1) {
  ...
  $result = pg_query(
      $conn,
      "select price from vouchertype where usage=$vouchertype"
  );
  ...
  $query
      = "INSERT INTO voucherclaims(vouchertype,voucherserial,claimserial,distributorno,patientid,serviceoffered,requiredfollowup,servicecharged,amount,remarks,claimdate,approval,attendant,profession,periodid,paymentdate,addedby)"
  ...
  $query
      = "UPDATE redeemedvouchers set claimed='Y' where redeemserial='$voucherserial' and vouchertype=$vouchertype and nationalid='$nationalid'";
  ...
  ?>
    <h4>Voucher can not be claimed because of Error with Redeeming.<br/><a
                href="vouchers_redeemed.php?period=<?php echo $periodid; ?>&distributor=<?php echo $distributor; ?>">
            Return to List</a><br/> <a href="trackvouchers_form.php">Track
            another Voucher</a></h4>
  <?php
}
pg_close();
?>
