...
if(in_array($vouchertype,array(5,6,7,9))){
    $finalredeem=0;
    $redeemStr="Voucher Partially Redeemed. The Matron / Sister-In-Charge should complete the process by redeeming the corresponding Cash Token";
}else{
    $finalredeem=1;
...
<?php
    $query = "insert into redeemedvouchers (nationalid,vouchertype,redeemserial,periodid,distributorno,redeemdate,ref,finalredeem,addedby) "
             . "values('$nationalid',$vouchertype,$redeemserial,$period,$distributor,'".date('d-m-Y')."','$ref',$finalredeem,'$username')";
    ...
    if(in_array($vouchertype,array(12,13))){
        $query = "update redeemedvouchers set finalredeem=1,periodid=$period where vouchertype in (5,6,7,9) and nationalid='$nationalid'";
        $result = pg_query($query); 
        //echo $query;
