...
if($voucherserial==''){$voucherserial=0;}
...
if($voucherserial==0){
    $sqlVoucher = pg_query($conn,"SELECT max(voucherserial)+1 FROM vouchersales");
...
if(strcmp($voucher,'update')==0){
    $query = "UPDATE vouchersales SET voucherserial=$voucherserial,saledate='$saledate',distributorno=$distributor"
    ...
    if($voucherduplicate>0){
        ...
        $query = "insert into vouchersales (voucherserial,nationalid,distributorno,saledate,addedby,dateadded) "
