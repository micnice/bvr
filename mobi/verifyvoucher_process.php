...
<html>
    ...
    <?php 
        ...
        if($refnumber>0){
            $sqlRef = pg_query($conn,"SELECT referralnumber From referrals where nationalid='$nationalid'");
        ...
        if($count==0){
            die("<div align='center'><br /><br />Beneficiary with National ID <em>" . $nationalid . "</em> does not exist!<br /><a href='verifyvoucher.php'>Try Again</a></div>");
        }else{
        }
    ?>
    <div align="left">
    ...
    <table style="margin:auto 0;">
    <?php
        $sqlStr = "SELECT vouchertype From redeemedvouchers where nationalid='$nationalid' and vouchertype=9";
        ...
        $sqlStr = "SELECT vouchertype From redeemedvouchers where nationalid='$nationalid' and vouchertype in (5,6,7,9)";
        ...
        if(strcmp($userrole, 'hospital')==0){
            $sqlStr1 = "SELECT * From vouchertype where usage not in (SELECT vouchertype from REDEEMEDVOUCHERS where nationalid='$nationalid') and usage in (7,9,14,15,16) order by usage asc";
        }elseif(strcmp($userrole, 'clinic')==0){
            $sqlStr1 = "SELECT * From vouchertype where usage not in (SELECT vouchertype from REDEEMEDVOUCHERS where nationalid='$nationalid') and usage in (1,2,3,4,5,6,10,11) order by usage asc";
        }elseif(strcmp($userrole, 'matron')==0 && $voucherDelivery==7){
            $sqlStr1 = "SELECT * From vouchertype where usage not in (SELECT vouchertype from REDEEMEDVOUCHERS where nationalid='$nationalid') and usage in (12) order by usage asc";    
        }elseif(strcmp($userrole, 'matron')==0 && $voucherDelivery==9){
            $sqlStr1 = "SELECT * From vouchertype where usage not in (SELECT vouchertype from REDEEMEDVOUCHERS where nationalid='$nationalid') and usage in (13) order by usage asc";    
        }elseif(strcmp($userrole, 'sic')==0 && ($voucherDelivery==5||$voucherDelivery==6)){
            $sqlStr1 = "SELECT * From vouchertype where usage not in (SELECT vouchertype from REDEEMEDVOUCHERS where nationalid='$nationalid') and usage in (12) order by usage asc";    
        }elseif(strcmp($userrole, 'ambulance')==0){
            $sqlStr1 = "SELECT * From vouchertype where usage not in (SELECT vouchertype from REDEEMEDVOUCHERS where nationalid='$nationalid') and usage in (8) order by usage asc";
        }elseif(strcmp($userrole, 'admin')==0){
            $sqlStr1 = "SELECT * From vouchertype where usage not in (SELECT vouchertype from REDEEMEDVOUCHERS where nationalid='$nationalid') order by usage asc";
        }
       //echo $sqlStr1.';';
