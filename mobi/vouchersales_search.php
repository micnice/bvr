...
if (strlen($nationalid) == 0) {
  ...
  $strSQL
      = "SELECT * From beneficiarymaster where (lower(trim(nationalid)) not in (select lower(trim(nationalid)) from vouchersales) or pregnancy=2) and lower(trim(nationalid)) like '%$nationalid%' limit 100";
}
...
$strBeneficiary
    = "SELECT * From beneficiarymaster where lower(trim(nationalid)) like '%$nationalid%' and length(trim(nationalid))>=10 limit 100";
...
if ($numrows == 0) {
  if ($numRowsBeneficiary == 0) {
    ...
    echo '<h3>'.$row["firstname"].' '.$row["surname"].', ID Number: '
        .$row["nationalid"].'</h2>';
    ?>
      <font color='red'><h3>Beneficiary Exists But has already bought a
              Voucher!</h3></font>
