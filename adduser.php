...
<form method="POST" action="main.php?page=adduser_process">
    <div align="center"><h3>Add New User</h3><br /><table  style="background-color: #E0F277;">
        <tr>
        ...
        <tr>
            ...
            <td><select name="userrole" id="userrole" required>
                ...
                $sql = pg_query($conn,"SELECT distinct userrole FROM users order by userrole asc");
        ...
        <tr>
            ...
            <td><select name="organisation" id="organisation" required>
                ...
                <?php
                include 'dbconnect.php';
                $sql = pg_query($conn,"SELECT distinct organisation FROM users order by organisation asc");
                while ($row = pg_fetch_assoc($sql)) {
                echo '<option value="'.htmlspecialchars($row['organisation']).'">'.htmlspecialchars($row['organisation']).'</option>';}
                //pg_close($conn);
                ?>
                </select> </td>
        </tr>
