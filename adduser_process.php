...
if(strcmp($adduser,'update')==0){
    $query = "UPDATE users SET username='$username', password=md5('$password'), firstname='$firstname',surname='$surname'"
...
}else if(strcmp($adduser,'new')==0){
    if(strcmp($facility,'0')==0){
        $query = "insert into users (username,password,firstname,surname,organisation,userrole) "
             . "values('$username',md5('$password'),'$firstname','$surname','$organisation','$userrole')";
    }else{
        $query = "insert into users (username,password,firstname,surname,organisation,userrole,facility) "
