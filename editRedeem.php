...
$redeemserial=trim($_GET['redeemserial']);
$vouchertype=trim($_GET['vouchertype']);

    $sql1 = pg_query($conn,"SELECT distributorno,periodid,nationalid From redeemedvouchers where redeemserial=$redeemserial and vouchertype=$vouchertype");
...
$sql = pg_query($conn,"SELECT voucherserial From vouchersales where nationalid='$nationalid'");
...
if($count==0){
    echo "<div align='center'><br /><br />No Voucher was sold to beneficiary with National ID <em>" . $nationalid . "</em>!";
 ?>
        <br />
        <?php
            if(strcmp($userrole, 'admin')==0 || strcmp($userrole, 'NPA')==0){
        ?>
        <a href="JavaScript:void(0);" onclick="openInParent('<?php echo $salerecordurl; ?>');">
          Enter Voucher Sale
        </a>
...
<?php
     $sql = pg_query($conn,"SELECT shortname From vouchertype where usage=$vouchertype");
        ...
        $label='Ref';
        if($vouchertype==1 || $vouchertype==2 || $vouchertype==3 || $vouchertype==4){ $label = 'ANC Register';  }
        if($vouchertype==5 || $vouchertype==6 || $vouchertype==7 || $vouchertype==9){ $label = 'Delivery Register';  }
        if($vouchertype==8){ $label = 'Referral Slip';  }
        if($vouchertype==10){ $label = 'PNC Register';  }
        if($vouchertype==11 || $vouchertype==12){ $redeemserial = '';  }
    ?>
        <br />Redeem <font color="red"><?php echo $shortname; ?> ?</font><br />
        
        <table style="margin:auto 0; border:0; margin:0; padding:0">
            <tr><td style="margin:0 auto">
            <form action="redeemvoucher_process.php" method="POST">
                <table>
                    <tr>
                        <td>Voucher/Token Serial:</td><td><input type="text" name='redeemserial' value="<?php echo $redeemserial ?>" <?php if($vouchertype==12 || $vouchertype==13){ echo ""; }else{ echo "readonly";  } ?>></td>
                    </tr>
                  
                    <tr>
                        <td><?php echo $label; ?> No. :</td><td><input type="text" size="11" maxlength="11" name="ref" value="<?php echo $ref; ?>" required></td>
                    </tr>
                </table>
